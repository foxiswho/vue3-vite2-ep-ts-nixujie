<template>
  <div v-if="!item.hidden" class="menu-wrapper">
    <template
      ...
      <el-menu-item :index="resolvePath2(onlyOneChild&&onlyOneChild.path?onlyOneChild.path:'')" :class="{'submenu-title-noDropdown':!isNest}">
        <i :class="[onlyOneChild.meta.icon||(item.meta&&item.meta.icon), 'icon']"></i>
        <template #title><span>{{ onlyOneChild.meta.title }}</span></template>
      </el-menu-item>
    </template>
    <el-submenu v-else ref="subMenu" :index="resolvePath2(item.path?item.path:'')">
      ...
      <sidebar-item
        ...
        :is-nest="true"
        :item="child"
        :base-path="resolvePath2(child.path?child.path:'')"
        class="nest-menu"
      />
    </el-submenu>
  </div>
